$(function() {
	...
	layui.table.render({
		...
		cols: [[
			...
			{field:'sysRealName', sort: true, title: '用户昵称'},
			{field:'sysName', sort: true, title: '用户名'},
            {field:'sysEmail', sort: true, title: '邮箱'},
			{field:'sysMobile', sort: true, title: '手机号'},
			{field:'sysCreateTime', sort: true, templet:function(d){ return layui.util.toDateString(d.sysCreateTime); }, title: '创建时间'},
			{field:'sysStatus', sort: true, templet: '#statusTpl',width: 80, title: '状态'},
...
function doDelete(obj){
	layer.confirm('确定要删除吗？', function(index){
		...
		layer.load(1);
		$.ajax({
			url: "sysUser/delete/"+obj.data.id+"?token="+getToken(),
			type: "DELETE", 
			dataType: "JSON", 
